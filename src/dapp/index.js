...
(async () => {
	...
	let contract = new Contract('localhost', () => {
			...
			contract.flights.forEach((flight) => {
				...
				if (flight.flight.statusCode == 0) {
					...
					let option = DOM.option({
						value: `${index} - ${price} ETH - ${flightRef} - ${from} - ${parseDate(
							+takeOff
						)} - ${to} - ${parseDate(+landing)}`,
					});
					datalist.appendChild(option);
					// append to oracle submission list
					datalist = DOM.elid('oracle-requests');
					option = DOM.option({
						value: `${flightRef} - ${to} - ${parseDate(+landing)}`,
					});
					datalist.appendChild(option);
				}
			});
		...
		DOM.elid('register-airline').addEventListener('click', async () => {
			...
			console.log('SELECTED', newAirline);
			await contract.registerAirline(newAirline);
			const { address, votes, error } = await contract.registerAirline(
				newAirline
			);
			display(`Airline ${sliceAddress(address)}`, 'Register Airline', [
				{
		...
		DOM.elid('buy').addEventListener('click', async () => {
			...
			const flight = contract.flights.filter((el) => {
				return el.index == index;
			});
			// These are all STRINGS
			const {
				flight: { flightRef, to, landing, price },
			} = flight[0];
			// execute transaction
			...
			display(`Passenger ${sliceAddress(passenger)}`, 'Book flight', [
				{
